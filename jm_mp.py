...
def find_description(x):
    mystring = r'[a-zA-WYZ]+[X]?[x]?'
    p = re.compile(mystring)
    m = p.findall(x)
    result = ' '.join(m)
    return result.lower()
...
def convert_date(x):
    if str(x) == 'nan':
        ...
        xx = x.split(' ')
        if 'PM' in xx:
            check_hour  = int(xx[1][:2])
            if check_hour <12:
                new_hour = str(check_hour+12)
                new_time = new_hour + xx[1][2:]
                new_date = ' '.join([xx[0],new_time,'PM'])
                return new_date
            else:
                return x
        else:
...
def calc_time_diff_in_hours(x):
    if x.isna().any():
        return -99
    elif x[1] == 'not_eligible':
        return -99
    else:
        start = datetime.strptime(x[0][:-3],'%m/%d/%Y %H:%M:%S')
        end = datetime.strptime(x[1][:-3],'%m/%d/%Y %H:%M:%S')
        time_diff = end - start
        return time_diff.seconds/3600

def calc_time_diff_in_days(x):
    if x.isna().any():
        return 'not_eligible'
    elif x[1] == 'not_eligible':
        return 'not_eligible'
...
def plot_bar(df,feature,target = 'Call_Flag',show_counts = False,show_annotate = True,rotation = 0,title_extra =''):
    ...
    ax.set_title('Barplot: {} vs {} '.format(target,feature)+title_extra,size = 20)
#     ax.set_ylim([0,0.66])
    ax.set_ylabel('{}'.format(target), size = 16)
...
def plot_heatmap(corr_mat,cmap="Blues"):
    ...
    sns.heatmap(corr_mat, mask = 1-mask,annot=True,linewidths=.5,cmap=cmap,ax = ax)
    plt.yticks(rotation=0)
    plt.xticks(rotation=25)
    plt.show()
    return fig

def plot_dist_numerical(df,feature,target):
    df['grp'] = pd.qcut(df[feature],q=10,duplicates = 'drop')
    dfgrp = df.groupby('grp')[[feature,target]].agg([np.mean,np.size])
    fig,ax = plt.subplots(figsize = (8,5))
    sns.distplot(df[feature],bins =30,kde = True, color = 'salmon',hist_kws ={'alpha':0.3})
    ax2 = ax.twinx()
    ax2.scatter(dfgrp[(feature,'mean')],dfgrp[(target,'mean')],color = 'lightblue',alpha = 0.6,s = dfgrp[(target,'size')]/500)
    ax.set_title('Mean {} vs Mean {}'.format(feature, target),size = 18)
    ax.set_xlabel('Mean {}'.format(feature),size = 14)
    ax.set_ylabel('Mean {}'.format(target),size = 14)              
    return fig


def create_accuracy_plots(df_s1,df_s2,num_bins = 10,labels = ['catboost','xgboost'],xlim =[0,1],do_regression = True,show_log_odd = True):
    ...
    if df_s1.shape[0]>0:
        if df_s1.shape[0]>0:
            ...
            ax[0].scatter(g_x,g_y, marker='o', linewidth=1,color = 'lightgreen'
                       ,alpha = 0.55,label = '{}'.format(labels[0]),s = df_scores_grp1[('score','size')]/60)

        if df_s2.shape[0]>0:
            g2_y  = df_scores_grp2[('y','mean')]
            g2_x  = df_scores_grp2[('score','mean')]
            ax[0].scatter(g2_x,g2_y, marker='o', linewidth=1,color = 'salmon'
                   ,alpha = 0.55,label = '{}'.format(labels[1]),s = df_scores_grp2[('score','size')]/60) 
        
        ax[0].plot([0,200],[0,200],linestyle = '--',color = 'grey',linewidth =5,label = 'ideal')
        ax[0].set_title('{} vs {}'.format(labels[0],labels[1]),size = 24)
        ax[0].set_xlabel('Predicted ',size = 16)
        ax[0].set_ylabel('Actual ',size = 16)
        ax[0].set_xlim(xlim)
        ax[0].set_ylim(xlim)
...
def mae(y,y_pred):
    """Return mean absolute error for regression"""
...
def mae_mse_df(y_test, y_preda, y_train, y_preda_train, name = 'index',round_decimals=3):
    mae_test         = mae(y_test, y_preda)
    mae_train        = mae(y_train, y_preda_train)
    mse_test         = mse(y_test, y_preda)
    mse_train        = mse(y_train, y_preda_train)    

    df_metrics = pd.DataFrame({
                                 'MAE': [mae_test, mae_train, mae_train - mae_test],
                               'MSE': [mse_test, mse_train, mse_train - mse_test]
                               },
